...
export class MenuService {
  ...
  updateMenuItem(id: string, item: Partial<MenuItem>) {
    return this.http.patch<{ data: { menuItem: MenuItem } }>(`/api/v1/menu/${id}`, item).pipe(map((response) => response.data.menuItem));
  }

  deleteMenuItem(id: string) { return this.http.delete(`/api/v1/menu/${id}`); }
